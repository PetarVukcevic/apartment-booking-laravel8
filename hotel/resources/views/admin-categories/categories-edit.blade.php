<x-layout>
    ...
    <section>
        <div class="container bg-gray p-md rounded">
            ...
            <form method="POST" action="/categories-edit/{{ $category->id }}"  name="edit_feature">
                @method('PATCH')
                @csrf
                <div class="form-group">
                    <label for="exampleFormControlInput1">Feature Title</label>
                    <input type="text" name="name" class="form-control" id="exampleFormControlInput1" value="{{ $category->name }}" placeholder="Title" required>
                    @error('name')
