<x-layout>
    ...
    <section>
        <div class="container bg-gray p-md rounded">
            ...
            <form method="POST" action="/cities-edit/{{ $city->id }}"  name="edit_city">
                @method('PATCH')
                @csrf
                <div class="form-group">
                    <label for="exampleFormControlInput1">Feature Title</label>
                    <input type="text" name="name" class="form-control" id="exampleFormControlInput1" value="{{ $city->name }}" placeholder="Title" required>
                    @error('name')
